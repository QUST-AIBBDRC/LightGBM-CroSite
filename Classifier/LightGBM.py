...
for train, test in loo.split(X,y): 
    cv_clf = lgb.LGBMClassifier(n_estimators=500,max_depth=15,learning_rate=0.2)
    X_train=X[train]
    y_train=y[train] 
    X_test=X[test]
    y_test=y[test]
...
yscore_sum.to_csv('y_score_s.csv')
ytest_sum = pd.DataFrame(data=y_sparse)
ytest_sum.to_csv('y_test_s.csv')
fpr, tpr, _ = roc_curve(y_sparse[:,0], y_score[:,0])
auc_score=result[6]
lw=2
