...
kw=  {'path': r"D:\Bioinformatics\chengxu\Feature_extraction_program\KNN",'train':r"D:\Bioinformatics\chengxu\Feature_extraction_program\KNN\data_n.txt",'label':r"D:\Bioinformatics\chengxu\Feature_extraction_program\KNN\label_test.txt",'order':'ACDEFGHIKLMNPQRSTVWY'}
...
data_pos=data_knn[:,np.array(range(1,column,2))]
test_pos=pd.DataFrame(data=data_pos)

data_neg=data_knn[:,np.array(range(0,column-1,2))]
test_neg=pd.DataFrame(data=data_neg)

test_pos.to_csv('data_pos.csv')
feature_pos = pd.read_csv('data_pos.csv', index_col=0)
test_neg.to_csv('data_neg.csv')
